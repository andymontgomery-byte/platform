...
| 14 | Implement a real hosted relational database. | GitHub Pages cannot host a database; the spec asks for database access as a real layer. | Supabase project provisioned; deploy-ready schema, seed data, read-only policies, and smoke queries are committed. Live load verification is pending credentials/SQL editor execution. | Postgres or equivalent is hosted with seeded demo data and read-only access path. |
| 15 | Implement a real hosted JSON API server. | Static JSON is not enough for filtering, auth, POSTs, or computed responses. | Static JSON plus local Express API; Supabase REST can provide basic read-only table access after seeding, but custom API behavior still needs a server layer. | Public API endpoints run at a real URL with OpenAPI matching behavior. |
