...
- 2026-04-27 06:15 ET: New rubric item `spec_fidelity_provable` landed with this PR. Total rubric is now 22 items. The seam is: on the 1EdTech side every field is a copy-exact projection of the spec (same object name, same field name, same JSON key) so there is never a silent rename; extensions live on our side of the seam and are tagged with `extension_rationale`. On first run the new generator detected 37 silent renames in `oneroster-core` (e.g., `personId` mapped to spec field `user`, `classId` mapped to `class`, `orgId` mapped to `org`, `lineItemId` mapped to `lineItem`). Fix in `data/data-dictionary.seed.json` by aligning `spec_field.column_name` and `spec_field.json_name` with `spec_field.sourceStandard.field`: `column_name = snake_case(sourceStandard.field)`, `json_name = sourceStandard.field` exactly. If a relational FK genuinely needs `_id` semantics, model it as a separate platform column with `sourceStandard.coverage = "platform_extension"` and add `extension_rationale`; do not alias it onto a 1EdTech field. Also fill `extension_rationale` on every `integration-governance-core` field (138 fields, all currently missing). Run `python3 scripts/generate_spec_fidelity_report.py --report-only` to regenerate the report; `--check` is the CI gate.
