...
| ID | Description | Why it matters | Blocked on | Owner | Target date |
|---|---|---|---|---|---|
| `PEND-001-runtime-slice-schedule` | Schedule the next runtime slices declared deferred by `DEC-012-runtime-coverage-per-spec`: CASE import/search beyond assignment URI alignment, QTI package import/projection, full Caliper Sensor API ingestion/projection beyond the build-guide event table, LTI Advantage services, production OAuth issuance, and broader privacy workflows. | DEC-012 decides the current posture, but the platform still needs dated delivery choices before any deferred Lead spec can be claimed as runtime-backed. | Product priority on next runtime slice | Andy | 2026-05-15 |
